...
local dumpsterModels = { -- Hier fügst du die Hash-Werte der Mülleimermodelle hinzu
    "prop_dumpster_01a",
    "prop_dumpster_02a"
}

local trashcanModels = { -- Hier fügst du die Hash-Werte der Mülleimernmodelle hinzu
    "prop_bin_01a",
    "prop_bin_07a"
}

RegisterNetEvent('qb-inventory:server:OpenInventory')
AddEventHandler('qb-inventory:server:OpenInventory', function(name, id, slots)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    if xPlayer ~= nil then
        local playerPed = GetPlayerPed(src)
        local coords = GetEntityCoords(playerPed)
        local dumpsterHash = GetHashKey(name)
        if dumpsterHash ~= 0 then
            for _, model in ipairs(dumpsterModels) do
                local modelHash = GetHashKey(model)
                if dumpsterHash == modelHash then
                    -- Hier kannst du die Logik hinzufügen, um das Inventar des Mülleimers zu öffnen
                    -- Zum Beispiel, um die Daten des Mülleimers zu laden und zu senden
        ...
        local trashcanHash = GetHashKey(name)
        if trashcanHash ~= 0 then
            for _, model in ipairs(trashcanModels) do
                local modelHash = GetHashKey(model)
                if trashcanHash == modelHash then
                    -- Hier kannst du die Logik hinzufügen, um das Inventar des Müllcontainers zu öffnen
                    -- Zum Beispiel, um die Daten des Müllcontainers zu laden und zu senden
                    TriggerClientEvent('qb-inventory:client:OpenInventory', src, {
                        label = "Trashcan",
                        slots = slots,
                        items = { },
